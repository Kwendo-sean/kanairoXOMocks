            ...
            child: Container(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  int segmentAt(double dx) =>
                      (dx / segmentWidth).floor().clamp(0, tabCount - 1);

                  return GestureDetector(
                    // One detector handles taps, the settings long-press AND
                    // horizontal swipes — so dragging that starts on top of an
                    // icon switches tabs just like dragging on empty space.
                    behavior: HitTestBehavior.opaque,
                    onTapUp: (details) => onTap(segmentAt(details.localPosition.dx)),
                    onLongPressStart: (details) {
                      if (segmentAt(details.localPosition.dx) == 4) {
                        onSettingsLongPress?.call();
                      }
                    },
